...
// const HORA = 10000
const HORA = 11114
const FECHA_PARTIDO_MANANA = 1071400000
const FECHA_PARTIDO_HOY = 1071300000 + HORA
...
async function main() {
    ...
    async function sacarPartidosMasPrimeraMitad() {
        ...
        for await (const line of rl) {
            ...
            if (line.includes('	Primera Mitad Total Goles Más/Menos de	Más de (0.5)	')) {
                // console.log(line)
                lineModificado = line.replace('	Primera Mitad Total Goles Más/Menos de	Más de (0.5)	', '***')
                lineModificado = lineModificado.replace('	 -', '')
                lineModificado = lineModificado.replace(' v ', '***')
                lineModificado = lineModificado.replace('	', '***')
    ...
    let partidos_optimizados = PartidosWplay.map(partido => {
        let cuotaCualquiera = PartidosSeMarcaraEnLaPrimeraMitad.find(p => partido.local === p[0] && partido.visitante === p[1]) ? PartidosSeMarcaraEnLaPrimeraMitad.find(p => partido.local === p[0] && partido.visitante === p[1])[2] : 1
        if (cuotaCualquiera === 1) {
            cuotaCualquiera = PartidosMasPrimeraMitad.find(p => partido.local === p[0] && partido.visitante === p[1]) ? PartidosMasPrimeraMitad.find(p => partido.local === p[0] && partido.visitante === p[1])[2] : 1
        }
        if (cuotaCualquiera === 1) {
            cuotaCualquiera = PartidosMasMenosPrimeraMitad.find(p => partido.local === p[0] && partido.visitante === p[1]) ? PartidosMasMenosPrimeraMitad.find(p => partido.local === p[0] && partido.visitante === p[1])[2] : 1
        }
        return {
            ...partido,
